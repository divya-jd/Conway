...
function grid(rows, cols) {
  ...
  $("tr").css("height", size);
  $("td").css("color", "gray").css("width", size);
  if (rows == 10 && cols == 10) {
...
$("#random").click(function () {
    ...
    sound.play();
    clearState();
    for (var i = 0; i < gridDimension*3; i++) {
      var randomValue = Math.floor(Math.random() * (gridDimension * gridDimension));
      $("#" + randomValue).addClass('HighLight');
    }
...
function increment(numOfIterations) {
  loadCurrentState();
  var sound = new Audio("mixkit-arcade-game-complete-or-approved-mission-205.wav");
  sound.play();
...
function unloadCurrentState() {
  ...
  for (var i = 0; i < gridDimension; i++) {
    for (var j = 0; j < gridDimension; j++) {
      if (
        currentState[i][j] === 1 &&
        !$("#" + (i * gridDimension + j)).hasClass("HighLight")
      ) {
